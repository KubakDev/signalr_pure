...
class MessagePackCodec extends Codec<Object?, Uint8List> {
  ...
  @override
  Object? decode(Uint8List encoded, [MessagePackDecodeOptions? options]) {
    if (options == null) {
      return decoder.convert(encoded);
    } else {
      return _MessagePackDecoder(_decoders, options).convert(encoded);
    }
  }

  @override
  Uint8List encode(Object? input, [MessagePackEncodeOptions? options]) {
    if (options == null) {
      return encoder.convert(input);
    } else {
      return _MessagePackEncoder(_encoders, options).convert(input);
    }
  }
}
